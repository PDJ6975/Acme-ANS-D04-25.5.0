...
public class TechnicianTaskListService extends AbstractGuiService<Technician, Task> {
	...
	public void load() {
		...
		// evitar mezcla incorrecta de parámetros
		if (maintenanceRecordId != null && mine) {
			super.getResponse().addGlobal("error", "No puedes combinar 'mine=true' con un 'maintenanceRecordId'.");
			super.getBuffer().addData(java.util.List.of()); // devolver lista vacía
			return;
		}
